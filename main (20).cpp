...
public:
    ...
    Punto() : x(0.0), y(0.0) {
        std::cout << "Punto creado en el origen (0,0) por constructor por defecto - Mateo Perez Nomey." << std::endl;
    }

    // Constructor 2: Con coordenadas específicas
    Punto(double coord_x, double coord_y) : x(coord_x), y(coord_y) {
        std::cout << "Punto creado en (" << x << "," << y << ") por constructor con coords - Mateo Perez Nomey." << std::endl;
    }
    ...
    Punto(const Punto& otroPunto) : x(otroPunto.x), y(otroPunto.y) {
        std::cout << "Punto copiado de (" << otroPunto.x << "," << otroPunto.y << ") - Mateo Perez Nomey." << std::endl;
    }
