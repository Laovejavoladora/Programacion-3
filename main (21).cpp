...
public:
    ...
    Motor(int c) : cilindros(c) {
        std::cout << "Motor(int) creado con " << cilindros << " cilindros - Mateo Perez Nomey" << std::endl;
    }

    // Constructor por defecto
    Motor() : cilindros(4) {
        std::cout << "Motor() por defecto creado con 4 cilindros - Mateo Perez Nomey" << std::endl;
    }
};
...
public:
    ...
    Auto(std::string m, int cil) 
        : marca(m), miMotor(cil) {
        std::cout << "Auto '" << marca << "' creado con motor de " << miMotor.cilindros << " cilindros - Mateo Perez Nomey" << std::endl;
    }
