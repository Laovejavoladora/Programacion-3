...
public:
    Empleado(string nom, int ed, string c) : Persona(nom, ed), cargo(c) {
        cout << "Empleado creado: " << nombre << ", Cargo: " << cargo << " - Mateo Perez Nomey" << endl;
    }
...
public:
    Caja(double l, double a) : largo(l), ancho(a) {
        cout << "Caja creada con dimensiones " << largo << " x " << ancho << " - Mateo Perez Nomey" << endl;
    }
};
...
public:
    CuentaBancaria(double inicial) {
        ...
        cout << "Cuenta bancaria creada con saldo inicial: " << saldo << " - Mateo Perez Nomey" << endl;
    }
